...
def batch_convert_pdf(output_folder, files_to_convert):
    #新增计数器
    total_files = len([f for f in files_to_convert if f.lower().endswith(".docx")])
    count = 0
...
def start_replace():
    ...
    for i, item in enumerate(replace_items):
        ...
        # 如果启用了渗透师，并且有选择
    if penetration_officer_var.get().strip():
       replace_dict["STS"] = penetration_officer_var.get().strip()
...
progress_bar.grid(row=btn_row+3, column=0, columnspan=3, padx=10, pady=10)

progress_label = tk.Label(root, text="0/0")
progress_label.grid(row=btn_row+4, column=0, columnspan=3, padx=10, pady=5)
# ---------- 添加进度条结束 ----------
